...
class DocxWidgetGenerator {
  ...
  List<Widget> generateWidgets(DocxBuiltDocument doc) {
    ...
    _shapeBuilder = ShapeBuilder(config: config, docxTheme: doc.theme);
  ...
  List<Widget> _generateContinuousWidgets(DocxBuiltDocument doc) {
    ...
    if (doc.section?.header != null) {
      widgets.addAll(
        _generateBlockWidgets(doc.section!.header!.children, counter: counter),
      );
    ...
    if (doc.section?.footer != null) {
      widgets.add(const Divider(height: 32, thickness: 1, color: Colors.grey));
      widgets.addAll(
        _generateBlockWidgets(doc.section!.footer!.children, counter: counter),
      );
    }
  ...
  Widget _buildPageContainer(
    ...
    if (doc.section?.header != null) {
      if (headerWidgets != null) {
        ...
        pageChildren.addAll(
          _generateBlockWidgets(doc.section!.header!.children),
        );
      }
  ...
  List<Widget> _generateBlockWidgets(
    ...
    while (i < elements.length) {
      ...
      if (element is DocxTable && element.position != null) {
        ...
        if (followingParagraphs.isNotEmpty) {
          ...
          final tableWidget = _tableBuilder.build(
            floatingTable,
            counter: counter,
          );
      ...
      if (element is DocxParagraph) {
        ...
        final activeLefts = localLefts
            .where((f) => !consumedFloats.contains(f))
            .toList();
        final activeRights = localRights
            .where((f) => !consumedFloats.contains(f))
            .toList();
        ...
        if (activeLefts.isNotEmpty) {
          ...
          while (j < elements.length && j < i + 5) {
            ...
            if (next is DocxParagraph) {
              ...
              // Inspect next rights
              final nextActiveRights = nextRights
                  .where((f) => !consumedFloats.contains(f))
                  .toList();
        ...
        if (activeLefts.isNotEmpty || finalRights.isNotEmpty) {
          ...
          final contentWidget = _paragraphBuilder.buildExcludingFloats(
            element,
            floatsToExclude,
            counter: counter,
          );
          ...
          if (localLefts.any((f) => consumedFloats.contains(f)) ||
            ...
            final floatsToExclude = {
              ...localLefts,
              ...localRights,
            }.where((f) => consumedFloats.contains(f)).toSet();

            widgets.add(
              _paragraphBuilder.buildExcludingFloats(
                element,
                floatsToExclude,
                counter: counter,
              ),
